...
export default function ChatScreen({ user }: ChatScreenProps) {
  ...
  useEffect(() => {
    if (!isLoading && messages.length >= 2 && sessionsLoaded) saveSession(messages, currentSessionId)
  ...
  const saveSession = useCallback(async (msgs: Message[], sessionId: string) => {
    ...
    setChatSessions(prev => {
      ...
      const updated  = [merged, ...prev.filter(s => s.id !== sessionId)].slice(0, 50)
    ...
    if (user) {
      try {
        ...
        await fetch('/api/chat/sessions', {
          ...
          body: JSON.stringify({ id: sessionId, title, messages: msgs, pinned: existing?.pinned ?? false, favorite: existing?.favorite ?? false }),
        })
      } catch { /* ignore */ }
    }
  }, [user, chatSessions])
  ...
  const loadSession = (session: ChatSession) => {
    ...
    setSidebarOpen(false); setInputValue(''); setAttachedFiles([])
    setActiveArtifact(null); setShowArtifactModal(false); setShowArtifactsList(false)
  }

  const newChat = useCallback(() => {
    if (messages.length >= 2) saveSession(messages, currentSessionId)
    setMessages([]); setInputValue(''); setAttachedFiles([]); setActiveChips(new Set())
    setSidebarOpen(false); setShowAttachMenu(false); setIsFocused(false)
    setActiveArtifact(null); setShowArtifactModal(false); setShowArtifactsList(false)
  ...
  const handleLongPressStart = (e: React.TouchEvent | React.MouseEvent, sessionId: string) => {
    ...
    longPressTimer.current = setTimeout(() => setContextMenu({ sessionId, x: rect.left, y: rect.bottom + 4 }), 500)
  }
  ...
  const handlePin = async (sessionId: string) => {
    const session = chatSessions.find(s => s.id === sessionId); const newPinned = !session?.pinned
    updateSessions(prev => prev.map(s => s.id === sessionId ? { ...s, pinned: newPinned } : s)); setContextMenu(null)
    if (user && session) { try { await fetch('/api/chat/sessions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...session, pinned: newPinned }) }) } catch { /* ignore */ } }
  }
  const handleFavorite = async (sessionId: string) => {
    const session = chatSessions.find(s => s.id === sessionId); const newFav = !session?.favorite
    updateSessions(prev => prev.map(s => s.id === sessionId ? { ...s, favorite: newFav } : s)); setContextMenu(null)
    if (user && session) { try { await fetch('/api/chat/sessions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...session, favorite: newFav }) }) } catch { /* ignore */ } }
  }
  const handleDelete = async (sessionId: string) => {
    updateSessions(prev => prev.filter(s => s.id !== sessionId))
    if (currentSessionId === sessionId) newChat(); setContextMenu(null)
    if (user) { try { await fetch('/api/chat/sessions', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: sessionId }) }) } catch { /* ignore */ } }
  }
  ...
  const confirmRename = async () => {
    ...
    if (user && newTitle) {
      ...
      if (session) { try { await fetch('/api/chat/sessions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...session, title: newTitle }) }) } catch { /* ignore */ } }
    }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length) setAttachedFiles(prev => [...prev, ...files]); e.target.value = ''
  }
  ...
  const handleMicClick = () => {
    ...
    rec.onerror = (e: SpeechRecognitionErrorEvent) => { setIsRecording(false); if (e.error === 'not-allowed') alert('Microphone permission denied.') }
  ...
  const handleSend = async () => {
    ...
    if (attachedFiles.length > 0) {
      ...
      for (const file of attachedFiles) {
        if (file.type.startsWith('image/')) { parts.push({ type: 'image_url', image_url: { url: await fileToBase64(file) } }) }
        else { try { parts.push({ type: 'text', text: `[File: ${file.name}]\n${await file.text()}` }) } catch { parts.push({ type: 'text', text: `[Attached file: ${file.name}]` }) } }
      }
      userContent = parts
    ...
    const { defaultModel } = loadSettings()

    try {
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ messages: newMessages, model: defaultModel }),
      })
      if (!res.ok) {
        const err = await res.json()
        setMessages([...newMessages, { role: 'assistant', content: err.error || 'Something went wrong.' }]); return
      }
        ...
        return (
          <div className="flex items-center gap-1.5 py-0.5">
            ...
            <span className="flex gap-[3px]">
              {[0, 1, 2].map(i => <span key={i} className="h-1.5 w-1.5 rounded-full bg-indigo-400" style={{ animation: `bounce 1.2s ease-in-out ${i * 0.2}s infinite` }} />)}
            </span>
        ...
        <div className="flex flex-col gap-3">
          {cleanText !== '' && (
            <span style={{ whiteSpace: 'pre-wrap' }}>
              {cleanText}
              {isAssistant && isLast && isStreaming && cleanText !== '' && <span className="ml-0.5 inline-block animate-pulse font-light text-gray-400">▌</span>}
            </span>
          )}
          ...
          {artifact && !isStreaming && (
            <button onClick={() => { setActiveArtifact(artifact); setShowArtifactModal(true) }}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11px] font-normal capitalize text-indigo-400">{artifact.type === 'code' ? artifact.language : artifact.type} · Tap to open</div>
              </div>
  ...
  // ── Input box ────────────────────────────────────────────────────────
  const InputBox = (
    <div className={`w-full rounded-3xl border p-4 shadow-sm transition-all duration-200 ${isActive ? 'border-[#4D6BFE]/60 bg-gradient-to-b from-[#EEF2FF] to-[#F0F4FF] shadow-lg shadow-[#4D6BFE]/10' : 'border-gray-200 bg-white'}`}>
      ...
      <textarea ref={textareaRef} placeholder={isRecording ? 'Grozl Is Listening...' : 'Ask Grozl anything...'}
      ...
      <div className="mt-3.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <button onClick={() => setShowAttachMenu(!showAttachMenu)} className="text-gray-500 transition hover:text-gray-700"><Plus className="h-5 w-5" /></button>
          </div>
    ...
    <div className="flex h-dvh overflow-hidden bg-gradient-to-b from-slate-50 to-indigo-50">
      <div className="flex w-full flex-col overflow-hidden">
        ...
        <div className={`fixed left-0 top-0 z-50 flex h-full w-72 -translate-x-full flex-col border-r border-gray-200 bg-white shadow-xl transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : ''}`}>
          <div className="flex flex-1 flex-col gap-2 overflow-y-auto p-6 pb-2">
            ...
            <button onClick={() => { setActiveMenuItem(activeMenuItem === 'newchat' ? null : 'newchat'); newChat() }}
              className={`flex items-center gap-3 rounded-xl border px-4 py-3 text-left text-[15px] font-medium transition-all ${activeMenuItem === 'newchat' ? 'border-[#4D6BFE]/60 bg-gradient-to-r from-[#EEF2FF] to-[#F0F4FF] text-[#4D6BFE] shadow-sm' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300 hover:bg-gray-50'}`}
            >
            ...
            <button onClick={() => setActiveMenuItem(activeMenuItem === 'projects' ? null : 'projects')}
              className={`flex items-center gap-3 rounded-xl border px-4 py-3 text-left text-[15px] font-medium transition-all ${activeMenuItem === 'projects' ? 'border-[#4D6BFE]/60 bg-gradient-to-r from-[#EEF2FF] to-[#F0F4FF] text-[#4D6BFE] shadow-sm' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300 hover:bg-gray-50'}`}
            >
              <FolderOpen className={`h-5 w-5 ${activeMenuItem === 'projects' ? 'text-[#4D6BFE]' : 'text-gray-400'}`} />
              Projects
            </button>
            <button onClick={() => { setActiveMenuItem(activeMenuItem === 'artifacts' ? null : 'artifacts'); setShowArtifactsList(prev => !prev) }}
              className={`flex items-center gap-3 rounded-xl border px-4 py-3 text-left text-[15px] font-medium transition-all ${activeMenuItem === 'artifacts' ? 'border-[#4D6BFE]/60 bg-gradient-to-r from-[#EEF2FF] to-[#F0F4FF] text-[#4D6BFE] shadow-sm' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300 hover:bg-gray-50'}`}
            >
              <svg className={`h-5 w-5 ${activeMenuItem === 'artifacts' ? 'text-[#4D6BFE]' : 'text-gray-400'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
                <rect x="3" y="3" width="7" height="7" rx="1" /><rect x="14" y="3" width="7" height="7" rx="1" />
                <rect x="3" y="14" width="7" height="7" rx="1" /><circle cx="17.5" cy="17.5" r="3.5" />
              </svg>
              Artifacts
              {allArtifacts.length > 0 && (
                <span className={`ml-auto rounded-full px-2 py-0.5 text-[11px] font-semibold ${activeMenuItem === 'artifacts' ? 'bg-[#4D6BFE]/20 text-[#4D6BFE]' : 'bg-gray-100 text-gray-500'}`}>{allArtifacts.length}</span>
              )}
            </button>
            {showArtifactsList && (
              <div className="ml-2 flex flex-col gap-1 border-l-2 border-indigo-100 pl-3">
                {allArtifacts.length === 0 ? <p className="py-2 text-[12px] italic text-gray-400">No artifacts yet</p> : allArtifacts.map((art, i) => (
                  <button key={i} onClick={() => { setActiveArtifact(art); setShowArtifactModal(true); setSidebarOpen(false) }}
            ...
            <div className="flex flex-col gap-1">
              {sortedSessions.length === 0 ? (
                <p className="px-2 py-3 text-[13px] italic text-gray-400">{searchQuery ? 'No matching chats' : 'No recent chats yet'}</p>
                  ...
                  {renamingId === session.id ? (
                    <div className="flex items-center gap-1.5 rounded-xl bg-indigo-50 px-3 py-2">
                      <input autoFocus value={renameValue} onChange={e => setRenameValue(e.target.value)}
                        ...
                        onBlur={confirmRename} className="flex-1 bg-transparent text-[14px] text-indigo-700 outline-none"
                      />
                    </div>
                  ) : (
                    <button onClick={() => loadSession(session)}
                      onMouseDown={e => handleLongPressStart(e, session.id)} onMouseUp={handleLongPressEnd} onMouseLeave={handleLongPressEnd}
                      onTouchStart={e => handleLongPressStart(e, session.id)} onTouchEnd={handleLongPressEnd}
                      onContextMenu={e => { e.preventDefault(); const rect = (e.currentTarget as HTMLElement).getBoundingClientRect(); setContextMenu({ sessionId: session.id, x: rect.left, y: rect.bottom + 4 }) }}
                    ...
                    >
                      <div className="mt-0.5 shrink-0">
                        {session.pinned ? <svg className="h-4 w-4 text-[#4D6BFE]" viewBox="0 0 24 24" fill="currentColor"><path d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/></svg>
                          : session.favorite ? <svg className="h-4 w-4 text-amber-400" viewBox="0 0 24 24" fill="currentColor"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
            ...
            <>
              ...
              >
                {[
                  ...
                  <button key={i} onClick={item.action} className="flex w-full items-center gap-3 border-b border-gray-100 px-4 py-3 text-[14px] font-medium text-gray-700 transition hover:bg-indigo-50 hover:text-indigo-600">{item.label}</button>
                ))}
                <button onClick={() => handleDelete(contextMenu.sessionId)} className="flex w-full items-center gap-3 px-4 py-3 text-[14px] font-medium text-rose-500 transition hover:bg-red-50">Delete</button>
              </div>
            </>
          )}
          ...
          <div className="flex items-center justify-between border-t border-gray-100 px-5 py-4">
            <div className="flex min-w-0 items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gray-900 text-[13px] font-bold text-white">{initials}</div>
              <span className="truncate text-[15px] font-medium text-gray-800">{displayName}</span>
            </div>
            <button onClick={() => { setSidebarOpen(false); setShowSettings(true) }} className="ml-3 shrink-0 rounded-xl p-2 text-gray-400 transition hover:bg-gray-100 hover:text-gray-700">
        ...
        <header className="absolute left-0 right-0 top-0 z-10 flex items-center justify-between p-4">
          <button onClick={() => setSidebarOpen(true)} className="text-gray-500 transition hover:text-gray-700"><Menu className="h-6 w-6" /></button>
          <button onClick={newChat} className="text-gray-500 transition hover:text-gray-700"><Plus className="h-6 w-6" /></button>
        </header>
        ...
        <main className="flex flex-1 flex-col overflow-hidden">
          {!hasMessages ? (
            <div className="flex flex-1 flex-col items-center justify-center px-4 pb-8">
              <div className="flex w-full max-w-[650px] flex-col items-center">
                <div className="mb-5 h-[90px] w-[90px]"><img src="/logo.png" alt="Grozl" className="h-full w-full object-contain" /></div>
                <h1 className="mb-7 text-center text-[28px] font-semibold tracking-tight text-gray-900">Your Mind, Amplified By Grozl</h1>
                {InputBox}
              </div>
            </div>
          ) : (
              ...
              <div className="mx-auto flex w-full max-w-[700px] flex-col gap-5">
                {messages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.role === 'assistant' && (
                      <div className="mr-2.5 mt-1 h-7 w-7 shrink-0 overflow-hidden rounded-full"><img src="/logo.png" alt="Grozl" className="h-full w-full object-contain" /></div>
                    )}
                    <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-[15px] leading-relaxed ${msg.role === 'user' ? 'bg-[#4D6BFE] text-white' : 'border border-gray-100 bg-white text-gray-800 shadow-sm'}`}>
          ...
          {hasMessages && <div className="w-full px-4 pb-4"><div className="mx-auto w-full max-w-[650px]">{InputBox}</div></div>}
        </main>
      </div>
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex flex-col md:hidden">
            <div className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3">
              ...
              <button onClick={() => { setActiveArtifact(null); setShowArtifactModal(false) }} className="text-gray-400 hover:text-gray-600"><X className="h-5 w-5" /></button>
            </div>
      ...
      {showSettings && (
        <SettingsScreen
          ...
          onClearChats={() => { setChatSessions([]); localStorage.removeItem('grozl_chat_sessions'); newChat() }}
          onLogout={() => { localStorage.removeItem('grozl_user_profile'); window.location.reload() }}
        />
      )}
    </div>
  )
