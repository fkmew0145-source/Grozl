...
export default function ChatScreen({ user }: ChatScreenProps) {
  ...
  // ── Dark mode init ───────────────────────────────────────────────────
  useEffect(() => {
    const stored = localStorage.getItem('grozl_dark_mode')
    if (stored === 'true') setIsDark(true)
  }, [])

  const toggleDark = () => {
    setIsDark(prev => {
      localStorage.setItem('grozl_dark_mode', String(!prev))
      return !prev
    })
  }

  // ── Load sessions (Supabase for logged-in, localStorage for guest) ───
  useEffect(() => {
    ...
    if (user && session) {
      try {
        await fetch('/api/chat/sessions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...session, pinned: newPinned }),
        })
      } catch { /* ignore */ }
    }
  }
    ...
    if (user && session) {
      try {
        await fetch('/api/chat/sessions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...session, favorite: newFav }),
        })
      } catch { /* ignore */ }
    }
  }
  ...
  const confirmRename = async () => {
    ...
    updateSessions(prev =>
      prev.map(s => s.id === renamingId ? { ...s, title: newTitle || s.title } : s)
    )
    if (user && newTitle) {
      ...
      if (session) {
        try {
          await fetch('/api/chat/sessions', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...session, title: newTitle }),
          })
        } catch { /* ignore */ }
      }
    }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // ── Trigger memory update in background (logged-in users only) ──
      if (user) {
      ...
      if (activeChips.has('think')) {
        return (
          <div className="flex items-center gap-1.5 py-0.5">
            <span className={`text-[13px] font-medium ${isDark ? 'text-indigo-400' : 'text-indigo-400'}`}>Thinking</span>
      ...
      return (
        <div className="flex flex-col gap-3">
          ...
          {isAssistant && isLast && isStreaming && !artifact && content.includes('<artifact') && (
            <div className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-[13px] ${isDark ? 'border-indigo-800 bg-indigo-950 text-indigo-400' : 'border-indigo-100 bg-indigo-50 text-indigo-500'}`}>
          ...
          {artifact && !isStreaming && (
            <button
              ...
              className={`flex items-center gap-2.5 rounded-xl border px-4 py-3 text-left text-[13px] font-medium transition hover:shadow-sm ${isDark ? 'border-indigo-700 bg-indigo-950/60 text-indigo-300 hover:border-indigo-600' : 'border-indigo-200 bg-gradient-to-r from-indigo-50 to-blue-50 text-indigo-700 hover:border-indigo-300'}`}
            >
              <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${isDark ? 'bg-indigo-800' : 'bg-indigo-100'}`}>
                <Code2 className="h-3.5 w-3.5 text-indigo-500" />
              </div>
              <div className="min-w-0 flex-1">
                <div className="truncate font-semibold">{artifact.title}</div>
                <div className={`text-[11px] font-normal capitalize ${isDark ? 'text-indigo-500' : 'text-indigo-400'}`}>
  ...
  // ── Theme classes ────────────────────────────────────────────────────
  const bg       = isDark ? 'bg-gray-950'  : 'bg-gradient-to-b from-slate-50 to-indigo-50'
  const cardBg   = isDark ? 'bg-gray-900'  : 'bg-white'
  const border   = isDark ? 'border-gray-700' : 'border-gray-200'
  const textMain = isDark ? 'text-gray-100' : 'text-gray-900'
  const textSub  = isDark ? 'text-gray-400' : 'text-gray-500'
  const inputBg  = isDark ? 'bg-gray-900 border-gray-700 text-gray-100 placeholder:text-gray-500' : 'bg-white border-gray-200 text-gray-800 placeholder:text-gray-400'
  ...
  const InputBox = (
    <div className={`w-full rounded-3xl border p-4 shadow-sm transition-all duration-200 ${
      isActive
        ? `border-[#4D6BFE]/60 shadow-lg shadow-[#4D6BFE]/10 ${isDark ? 'bg-gray-900' : 'bg-gradient-to-b from-[#EEF2FF] to-[#F0F4FF]'}`
        : `${border} ${cardBg}`
    }`}>
      ...
      {attachedFiles.length > 0 && (
        <div className="mb-2 flex flex-wrap gap-2">
          {attachedFiles.map((file, i) => (
            <div key={i} className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-[13px] ${isDark ? 'border-indigo-800 bg-indigo-950 text-indigo-300' : 'border-indigo-100 bg-indigo-50 text-indigo-700'}`}>
      ...
      <textarea
        ...
        disabled={isLoading}
        className={`w-full resize-none bg-transparent text-base outline-none disabled:opacity-50 ${isDark ? 'text-gray-100 placeholder:text-gray-500' : 'text-gray-800 placeholder:text-gray-400'}`}
      />
            ...
            <button
              ...
              className={`flex items-center gap-2 rounded-full border px-4 py-2 text-[13px] font-medium transition-all duration-250 ease-out capitalize ${
                activeChips.has(chip)
                  ? `border-[#4D6BFE]/60 text-[#4D6BFE] shadow-sm shadow-[#4D6BFE]/10 ${isDark ? 'bg-indigo-950' : 'bg-gradient-to-r from-[#EEF2FF] to-[#F0F4FF]'}`
                  : `${border} ${isDark ? 'bg-gray-800 text-gray-400 hover:text-gray-200' : 'bg-white/80 text-gray-500 hover:border-gray-300 hover:bg-white hover:text-gray-600 hover:shadow-sm'}`
              }`}
            >
              {chip === 'think' ? (
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            {showAttachMenu && (
              <>
                ...
                <div className={`absolute bottom-9 right-0 z-50 w-[160px] overflow-hidden rounded-2xl border shadow-2xl ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
                  {[
                    ...
                    <button
                      key={item.label}
                      onClick={() => { item.ref.current?.click(); setShowAttachMenu(false) }}
                      className={`flex w-full items-center gap-3 px-4 py-3 text-[14px] font-medium transition hover:bg-indigo-50 hover:text-indigo-600 ${idx < arr.length - 1 ? `border-b ${border}` : ''} ${textMain}`}
                    >
            ...
            <button onClick={() => setShowAttachMenu(!showAttachMenu)} className={`transition ${textSub} hover:${textMain}`}>
          ...
          {!hasText && attachedFiles.length === 0 ? (
            <button
              ...
              className={`transition ${isRecording ? 'animate-pulse text-red-500' : `${textSub} hover:${textMain}`}`}
            >
  ...
  return (
    <div className={`flex h-dvh overflow-hidden ${bg}`}>
      ...
      <div className="flex w-full flex-col overflow-hidden">
        ...
        {/* ── Sidebar ──────────────────────────────────────────────────── */}
        <div className={`fixed left-0 top-0 z-50 flex h-full w-72 -translate-x-full flex-col gap-2 border-r p-6 shadow-xl transition-transform duration-300 ${sidebarBg} ${sidebarOpen ? 'translate-x-0' : ''}`}>
          {/* Search */}
          <div className="relative mb-5">
            <Search className={`absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 ${textSub}`} />
            <input
              type="text"
              placeholder="Search chats..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className={`w-full rounded-xl border py-2.5 pl-9 pr-3 text-sm outline-none focus:border-indigo-300 ${inputBg}`}
            />
          </div>
          ...
          <button
            ...
            className={`flex items-center gap-3 rounded-xl border px-4 py-3 text-left text-[15px] font-medium transition-all duration-200 ease-out ${
              activeMenuItem === 'newchat'
                ? `border-[#4D6BFE]/60 text-[#4D6BFE] shadow-sm shadow-[#4D6BFE]/10 ${isDark ? 'bg-indigo-950' : 'bg-gradient-to-r from-[#EEF2FF] to-[#F0F4FF]'}`
                : `${border} ${cardBg} ${textSub} hover:${textMain}`
            }`}
          >
            <MessageSquarePlus className={`h-5 w-5 ${activeMenuItem === 'newchat' ? 'text-[#4D6BFE]' : textSub}`} />
            New Chat
          </button>
          ...
          >
            <FolderOpen className={`h-5 w-5 ${activeMenuItem === 'projects' ? 'text-[#4D6BFE]' : textSub}`} />
            Projects
          </button>

          {/* Recent Chats */}
          <span className={`ml-1 mt-5 text-xs font-semibold uppercase tracking-wide ${textSub}`}>Recent Chats</span>
          <div className="flex flex-1 flex-col gap-1 overflow-y-auto">
            {sortedSessions.length === 0 ? (
              <p className={`px-2 py-3 text-[13px] italic ${textSub}`}>
              ...
              sortedSessions.map(session => (
                <div key={session.id} className="relative">
                  {renamingId === session.id ? (
                    <div className={`flex items-center gap-1.5 rounded-xl px-3 py-2 ${isDark ? 'bg-indigo-950' : 'bg-indigo-50'}`}>
                      <input
                        autoFocus
                        value={renameValue}
                        ...
                        className={`flex-1 bg-transparent text-[14px] outline-none ${isDark ? 'text-indigo-300' : 'text-indigo-700'}`}
                      />
                    </div>
                  ) : (
                    <button
                      ...
                      className={`flex w-full items-start gap-2.5 rounded-xl px-3 py-2.5 text-left text-[14px] transition hover:bg-indigo-50 hover:text-indigo-600 ${
                    ...
                    >
                      <div className="mt-0.5 shrink-0">
                        {session.pinned ? (
                          ...
                          <MessageSquare className={`h-4 w-4 ${textSub}`} />
                        )}
                      </div>
          ...
          {contextMenu && (
            <>
              ...
              <div
                className={`fixed z-[70] w-44 overflow-hidden rounded-2xl border shadow-2xl ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}
                style={{
              ...
              >
                {[
                  { label: chatSessions.find(s => s.id === contextMenu.sessionId)?.pinned ? 'Unpin' : 'Pin',       action: () => handlePin(contextMenu.sessionId),      color: 'indigo' },
                  { label: chatSessions.find(s => s.id === contextMenu.sessionId)?.favorite ? 'Unfavorite' : 'Favorite', action: () => handleFavorite(contextMenu.sessionId), color: 'amber' },
                  { label: 'Rename', action: () => { const s = chatSessions.find(x => x.id === contextMenu.sessionId); if (s) startRename(s) }, color: 'indigo' },
                ].map((item, i) => (
                  <button key={i} onClick={item.action} className={`flex w-full items-center gap-3 border-b ${border} px-4 py-3 text-[14px] font-medium ${textMain} transition hover:bg-indigo-50 hover:text-indigo-600`}>
                    {item.label}
                  </button>
                ))}
          ...
          <button
            onClick={toggleDark}
            className={`mt-1 flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition ${border} ${isDark ? 'text-yellow-400 hover:bg-yellow-950' : 'text-gray-600 hover:bg-gray-50'}`}
          >
          ...
          {/* Sign Out */}
          {user && (
            <button onClick={handleSignOut} className="mt-1 flex items-center gap-2 rounded-xl bg-red-50 px-4 py-2.5 text-left text-sm font-medium text-rose-600 transition hover:bg-red-100">
        ...
        <header className="absolute left-0 right-0 top-0 z-10 flex items-center justify-between p-4">
          <button onClick={() => setSidebarOpen(true)} className={`transition ${textSub}`}>
            <Menu className="h-6 w-6" />
          </button>
          <button onClick={newChat} className={`transition ${textSub}`}>
          ...
          {!hasMessages ? (
            <div className="flex flex-1 flex-col items-center justify-center px-4 pb-8">
              <div className="flex w-full max-w-[650px] flex-col items-center">
                ...
                <h1 className={`mb-7 text-center text-[28px] font-semibold tracking-tight ${textMain}`}>
                  ...
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-[15px] leading-relaxed ${
                      msg.role === 'user'
                        ...
                        : `border shadow-sm ${isDark ? 'border-gray-700 bg-gray-800 text-gray-100' : 'border-gray-100 bg-white text-gray-800'}`
                    }`}>
      ...
      {/* ── Artifact Modal (full-screen on mobile, panel on desktop) ──── */}
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex flex-col md:hidden">
            <div className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3">
              ...
              <button
                onClick={() => { setActiveArtifact(null); setShowArtifactModal(false) }}
