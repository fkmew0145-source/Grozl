...
export default function ChatScreen({ user }: ChatScreenProps) {
  ...
  // ── Persist settings ─────────────────────────────────────────────────
  useEffect(() => {
    localStorage.setItem('grozl_settings', JSON.stringify({
      appearance: settingsAppearance,
      fontSize: settingsFontSize,
      language: settingsLanguage,
      defaultModel: settingsDefaultModel,
      saveHistory: settingsSaveHistory,
    }))
  }, [settingsAppearance, settingsFontSize, settingsLanguage, settingsDefaultModel, settingsSaveHistory])

  // ── Apply font size to root ──────────────────────────────────────────
  useEffect(() => {
    const sizes = { small: '14px', medium: '16px', large: '18px' }
    document.documentElement.style.setProperty('--chat-font-size', sizes[settingsFontSize])
  }, [settingsFontSize])

  // ── Load memory when settings opens ─────────────────────────────────
    ...
    <div className="flex h-dvh overflow-hidden bg-gradient-to-b from-slate-50 to-indigo-50">
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center">
          ...
          {/* Modal */}
          <div className="relative z-10 flex w-full max-w-md flex-col overflow-hidden rounded-t-3xl sm:rounded-3xl bg-white shadow-2xl max-h-[92vh]">

            {/* Header */}
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <h2 className="text-[17px] font-semibold text-gray-900">Settings</h2>
              <button onClick={() => setShowSettings(false)} className="rounded-xl p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                <X className="h-5 w-5" />
              </button>
            </div>

            {/* Tab Nav */}
            <div className="flex gap-1 overflow-x-auto px-4 py-3 scrollbar-hide border-b border-gray-100">
              {([
                ...
                <button key={tab.id} onClick={() => setSettingsTab(tab.id)}
                  className={`shrink-0 rounded-xl px-3 py-1.5 text-[13px] font-medium transition ${
                    settingsTab === tab.id
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5">
              ...
              {settingsTab === 'profile' && (
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col items-center gap-2 rounded-2xl bg-gray-50 px-4 py-5">
                    <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gray-900 text-2xl font-bold text-white">
                      {(userProfile?.nickname || userProfile?.fullName || (user?.email ?? 'G'))[0].toUpperCase()}
                    </div>
                    <p className="text-[16px] font-semibold text-gray-900">{userProfile?.fullName || 'Guest User'}</p>
                    {userProfile?.nickname && <p className="text-[13px] text-gray-400">@{userProfile.nickname}</p>}
                    {user?.email && <p className="text-[12px] text-gray-400">{user.email}</p>}
                  </div>
                  <div className="rounded-2xl bg-gray-50 px-4 py-3">
                    <p className="mb-1 text-[12px] font-semibold uppercase tracking-wide text-gray-400">Account type</p>
                    <p className="text-[15px] font-medium text-gray-800">{user ? '✅ Logged In' : '👤 Guest'}</p>
                  </div>
                  {!user && (
                    <div className="rounded-2xl border border-[#4D6BFE]/20 bg-[#EEF2FF] px-4 py-3">
                      <p className="text-[13px] text-[#4D6BFE]">Sign in to sync your chats & memory across devices.</p>
                    </div>
                  )}
                  <button
                    onClick={() => { setShowSettings(false); localStorage.removeItem('grozl_user_profile'); window.location.reload() }}
                    className="flex w-full items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 py-3 text-[14px] font-medium text-red-500 transition hover:bg-red-100"
                  >
                    🚪 Log Out
                  </button>
                </div>
              ...
              {settingsTab === 'memory' && (
                <div className="flex flex-col gap-4">
                  <div className="rounded-2xl bg-blue-50 px-4 py-3">
                    <p className="text-[13px] text-blue-700">Grozl remembers things you share — name, preferences, projects etc. Ye memory har conversation mein use hoti hai.</p>
                  </div>
                  {!user ? (
                    <div className="rounded-2xl bg-gray-50 px-4 py-4 text-center">
                      <p className="text-[14px] text-gray-500">Sign in to use persistent memory.</p>
                    </div>
                  ) : (
                    <>
                      <div className="flex flex-col gap-1">
                        <label className="text-[12px] font-semibold uppercase tracking-wide text-gray-400">Current Memory</label>
                        <textarea
                          value={memoryText}
                          onChange={e => setMemoryText(e.target.value)}
                          rows={8}
                          placeholder={memoryLoading ? 'Loading...' : 'Koi memory nahi abhi...'}
                          disabled={memoryLoading}
                          className="w-full resize-none rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-[14px] text-gray-800 outline-none focus:border-[#4D6BFE] focus:bg-white transition"
                        />
                      </div>
                      <div className="flex gap-2">
                        <button onClick={saveMemory} disabled={memoryLoading}
                          className="flex flex-1 items-center justify-center gap-1.5 rounded-2xl bg-[#4D6BFE] py-3 text-[14px] font-medium text-white transition hover:bg-[#3D5BEE] disabled:opacity-60"
                        >
                          {memorySaved ? '✅ Saved!' : memoryLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : '💾 Save Memory'}
                        </button>
                        <button onClick={clearMemory} disabled={memoryLoading}
                          className="flex items-center justify-center gap-1.5 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-[14px] font-medium text-red-500 transition hover:bg-red-100"
                        >
                          🗑️ Clear
                        </button>
                      </div>
              ...
              {settingsTab === 'model' && (
                <div className="flex flex-col gap-3">
                  <p className="text-[13px] text-gray-500">Choose default AI model. <span className="font-medium text-[#4D6BFE]">Auto</span> = smart routing (recommended).</p>
                  {([
                    { id: 'auto',     label: '⚡ Auto (Smart Routing)', desc: 'Best model chosen per query' },
                    { id: 'deepseek', label: '🧠 DeepSeek R1',          desc: 'Best for code & math' },
                    { id: 'groq',     label: '🦙 Groq Llama 3.3',       desc: 'Fast, great for chat' },
                    { id: 'gemini',   label: '👁️ Gemini Flash',          desc: 'Vision + general tasks' },
                  ] as const).map(m => (
                    <button key={m.id} onClick={() => setSettingsDefaultModel(m.id)}
                      className={`flex items-center gap-3 rounded-2xl border px-4 py-3.5 text-left transition ${
                        settingsDefaultModel === m.id
                          ? 'border-[#4D6BFE]/40 bg-[#EEF2FF]'
                          : 'border-gray-200 bg-gray-50 hover:bg-gray-100'
                      }`}
                    >
                      <div className={`h-5 w-5 shrink-0 rounded-full border-2 flex items-center justify-center ${settingsDefaultModel === m.id ? 'border-[#4D6BFE]' : 'border-gray-300'}`}>
                        {settingsDefaultModel === m.id && <div className="h-2.5 w-2.5 rounded-full bg-[#4D6BFE]" />}
                      </div>
                      <div>
                        <p className={`text-[14px] font-medium ${settingsDefaultModel === m.id ? 'text-[#4D6BFE]' : 'text-gray-800'}`}>{m.label}</p>
              ...
              {settingsTab === 'appearance' && (
                <div className="flex flex-col gap-5">
                  {/* Theme */}
                  <div>
                    <label className="mb-2 block text-[12px] font-semibold uppercase tracking-wide text-gray-400">Theme</label>
                    <div className="grid grid-cols-3 gap-2">
                      {([
                        ...
                        <button key={t.id} onClick={() => setSettingsAppearance(t.id)}
                          className={`rounded-2xl border py-3 text-[13px] font-medium transition ${
                            settingsAppearance === t.id
                    ...
                    {settingsAppearance !== 'system' && (
                      <p className="mt-2 text-[12px] text-amber-600">⚠️ Dark mode UI coming soon — Light mode is active now.</p>
                    )}
                  </div>
                  {/* Font Size */}
                  <div>
                    <label className="mb-2 block text-[12px] font-semibold uppercase tracking-wide text-gray-400">Font Size</label>
                    <div className="grid grid-cols-3 gap-2">
                      {([
                        { id: 'small',  label: 'A',  sub: 'Small'  },
                        { id: 'medium', label: 'A',  sub: 'Medium' },
                        { id: 'large',  label: 'A',  sub: 'Large'  },
                      ] as const).map((f, i) => (
                        <button key={f.id} onClick={() => setSettingsFontSize(f.id)}
                          className={`flex flex-col items-center rounded-2xl border py-3 transition ${
                            settingsFontSize === f.id
                              ? 'border-[#4D6BFE] bg-[#EEF2FF] text-[#4D6BFE]'
                              : 'border-gray-200 bg-gray-50 text-gray-600 hover:bg-gray-100'
                          }`}
                        >
                          <span style={{ fontSize: `${14 + i * 3}px` }} className="font-bold">{f.label}</span>
                          <span className="text-[11px] mt-0.5">{f.sub}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                  {/* Language */}
                  <div>
                    <label className="mb-2 block text-[12px] font-semibold uppercase tracking-wide text-gray-400">Response Language</label>
                    <div className="grid grid-cols-3 gap-2">
                      {([
                        ...
                        <button key={l.id} onClick={() => setSettingsLanguage(l.id)}
                          className={`rounded-2xl border py-3 text-[12px] font-medium transition ${
                            settingsLanguage === l.id
                              ? 'border-[#4D6BFE] bg-[#EEF2FF] text-[#4D6BFE]'
                              : 'border-gray-200 bg-gray-50 text-gray-600 hover:bg-gray-100'
                          }`}
                        >{l.label}</button>
                    ...
                    <p className="mt-2 text-[12px] text-gray-400">Grozl waise bhi aapki language mirror karta hai — ye default set karta hai.</p>
                  </div>
                </div>
              )}
              ...
              {settingsTab === 'history' && (
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-gray-50 px-4 py-4">
                    <div>
                      <p className="text-[14px] font-medium text-gray-800">Save Chat History</p>
                      <p className="text-[12px] text-gray-400">Conversations save karo locally</p>
                    </div>
                    <button
                      onClick={() => setSettingsSaveHistory(p => !p)}
                      className={`relative h-6 w-11 rounded-full transition-colors ${settingsSaveHistory ? 'bg-[#4D6BFE]' : 'bg-gray-300'}`}
                    >
                      <span className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform ${settingsSaveHistory ? 'translate-x-5' : 'translate-x-0.5'}`} />
                    </button>
                  </div>
                  {/* Stats */}
                  <div className="rounded-2xl bg-gray-50 px-4 py-3">
                    <p className="text-[12px] font-semibold uppercase tracking-wide text-gray-400">Saved Sessions</p>
                    <p className="mt-1 text-[22px] font-bold text-gray-900">{chatSessions.length} <span className="text-[14px] font-normal text-gray-400">chats</span></p>
                  </div>
                  {/* Clear all */}
                  <button
                    onClick={() => {
                      if (!confirm('Saare chats delete karne hain? Ye undo nahi hoga.')) return
                       setChatSessions([])
              ...
              {settingsTab === 'about' && (
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col items-center gap-2 rounded-2xl bg-gradient-to-br from-[#EEF2FF] to-[#F0F4FF] px-4 py-6">
                    <img src="/logo.png" alt="Grozl" className="h-16 w-16 object-contain" />
                    <p className="text-[18px] font-bold text-gray-900">Grozl AI</p>
                    <p className="text-[12px] text-gray-400">Version 1.0.0</p>
                    <p className="text-center text-[13px] text-gray-500 mt-1">All the World's Best AI. One Platform.</p>
                  </div>
                  <div className="flex flex-col gap-2">
                    {[
                      { label: '🤖 Models Used',     value: 'DeepSeek R1 · Groq Llama 3.3 · Gemini 1.5 Flash' },
                      { label: '🏗️ Built With',       value: 'Next.js 16 · React 19 · Supabase · Tailwind v4'  },
                      { label: '⚡ Rate Limit',        value: '30 requests / minute'                             },
                      { label: '🔒 Auth',              value: 'Supabase OAuth (Google)'                          },
                    ].map((item, i) => (
                      <div key={i} className="rounded-2xl bg-gray-50 px-4 py-3">
                        <p className="text-[12px] font-semibold text-gray-400">{item.label}</p>
                        <p className="mt-0.5 text-[13px] text-gray-700">{item.value}</p>
                      </div>
                    ))}
                  </div>
                  <a href="mailto:grozlteam@gmail.com"
                    className="flex w-full items-center justify-center gap-2 rounded-2xl border border-gray-200 bg-gray-50 py-3 text-[14px] font-medium text-gray-600 transition hover:bg-gray-100"
                  >
                    📧 Help & Feedback
                  </a>
                  <p className="text-center text-[11px] text-gray-400">AI-generated content is for reference only. Use legally.</p>
                </div>
              )}
